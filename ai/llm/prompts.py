...
def build_plan_prompt(decision_input: DecisionInput) -> str:
    ...
    return (
        ...
        f"{_result_block(decision_input)}\n\n"
        f"{_findings_block(decision_input)}\n\n"
        f"Task: Generate a full ordered cyber kill-chain plan from {phase} to PROOF_OF_COMPROMISE.\n"
        f"- Cover the remaining phases, not just the next action.\n"
        f"- Return 3 to 6 compact steps.\n"
        ...
        f"- Keep the entire response short enough to fit in one JSON object.\n"
        f"{_target_lock_rules(decision_input)}\n\n"
        f"Phases remaining:\n{phase_guide}\n\n"
